...
use ark_snark::SNARK;
use ark_sponge::{
    poseidon::{PoseidonSponge, PoseidonParameters, constraints::PoseidonSpongeVar},
    CryptographicSponge, constraints::CryptographicSpongeVar,
};
...
use rsa::{
    bigint::{nat_to_f, BigInt, constraints::{BigIntCircuitParams, BigIntVar}},
    hog::{RsaGroupParams, RsaHiddenOrderGroup, constraints::RsaHogVar},
    ...
    GV: CurveVar<G, F>,
{
    ...
    pub fn commit<R: CryptoRng + Rng>(
        ...
        // TODO: Switch back to 128
        let r = BigInt::from(rng.gen_biguint(32));
...
#[derive(Clone, PartialEq, Eq, Debug)]
pub struct TCCircuit<F, P, RsaP, IntP, G, GV> where
F: PrimeField,
P: SnarkTCParams<F>,
RsaP: RsaGroupParams,
IntP: BigIntCircuitParams,
    ...
    GV: CurveVar<G, F>,
{
    fn generate_constraints(self, cs: ConstraintSystemRef<F>) -> Result<(), SynthesisError> {
        ...
        let modulus_bits = RsaP::M.deref().bits() as usize;
        ...
        let comm_x = RsaHogVar::<F, RsaP, IntP>::new_input(
            ark_relations::ns!(cs, "comm_x"),
            || Ok(&self.comm.x),
        )?;
        let comm_ped = <GV as AllocVar<G, F>>::new_input(
            ark_relations::ns!(cs, "comm_ped"),
            || Ok(&self.comm.ped_comm),
        )?;
        let comm_ct = <Vec<UInt8<F>>>::new_witness(
            ark_relations::ns!(cs, "comm_ct"),
            || Ok(self.comm.ct),
        )?;

        // Allocate witness inputs
        let ped_opening = <Vec<Boolean<F>>>::new_witness(
            ark_relations::ns!(cs, "ped_opening"),
            || Ok(self.ped_opening.into_repr().to_bits_le()),
        )?;
        //TODO: Change witness to bits input
        let tc_r = BigIntVar::<F, IntP>::new_witness(
            ark_relations::ns!(cs, "tc_randomizer"),
            || Ok(&self.time_pp_r),
        )?;
        debug_assert_eq!(self.m.len(), P::M_LEN);
        let m = <Vec<UInt8<F>>>::new_witness(
            ark_relations::ns!(cs, "tc_randomizer"),
            || Ok(self.m),
        )?;

        // Generate constraints
        let computed_ped_comm = g.scalar_mul_le(m.to_bits_le()?.iter())? + h.scalar_mul_le(ped_opening.iter())?;
        comm_ped.enforce_equal(&computed_ped_comm)?;

        let computed_tc_x = x.power(&tc_r, &modulus, modulus_bits)?;
        comm_x.enforce_equal(&computed_tc_x)?;

        let tc_y = y.power(&tc_r, &modulus, modulus_bits)?;
        hasher.absorb(&tc_y.to_bytes()?)?;
        let pad = hasher.squeeze_bytes(P::M_LEN)?;
        let computed_ct = pad.iter().zip(m.iter())
            .map(|(x1, x2)| x1.xor(x2))
...
impl<F, P, RsaP, IntP, G, GV> Default for TCCircuit<F, P, RsaP, IntP, G, GV>
    where
        F: PrimeField,
        P: SnarkTCParams<F>,
        RsaP: RsaGroupParams,
        ...
        GV: CurveVar<G, F>,
{
    fn default() -> Self {
        ...
        Self {
            time_params: TimeParams { t: 0, x: Default::default(), y: Default::default() },
            ped_params: PedersenParams { g: Default::default(), h: Default::default() },
            comm: Comm {
...
#[derive(Clone, PartialEq, Eq, Debug)]
pub struct TCCircuitPublicInput<RsaP, IntP, G> where
    RsaP: RsaGroupParams,
    IntP: BigIntCircuitParams,
    G: ProjectiveCurve,
{
    ...
    G: ProjectiveCurve + ToConstraintField<F>,
{
    fn to_field_elements(&self) -> Option<Vec<F>> {
        ...
        out.append(&mut nat_to_limbs(&self.time_params.y.n, IntP::LIMB_WIDTH, IntP::N_LIMBS).unwrap());
...
mod tests {
    use super::*;
    use ark_ed_on_bls12_381::{EdwardsProjective as G, constraints::EdwardsVar as GV};
    ...
    pub type TC = SnarkTC<F, Groth16<Bls12_381>, TestSnarkTCParams, TestPoEParams, TestRsaParams, BigNatTestParams, Sha3_256, G, GV>;

    pub type Circuit512 = TCCircuit<F, TestSnarkTCParams, TestRsa512Params, BigNat512TestParams, G, GV>;
    pub type TC512 = SnarkTC<F, Groth16<Bls12_381>, TestSnarkTCParams, TestPoEParams, TestRsa512Params, BigNat512TestParams, Sha3_256, G, GV>;

    pub type Circuit64 = TCCircuit<F, TestSnarkTCParams, TestRsa64Params, BigNat64TestParams, G, GV>;
    pub type TC64 = SnarkTC<F, Groth16<Bls12_381>, TestSnarkTCParams, TestPoEParams, TestRsa64Params, BigNat64TestParams, Sha3_256, G, GV>;
    ...
    fn snark_tc_64_test() {
        ...
        let (pk, vk) = Groth16::<Bls12_381>::circuit_specific_setup(Circuit64::default(), &mut rng).unwrap();


        let (comm, self_opening, proof) = TC64::commit(&mut rng, &time_pp, &ped_pp, &pk, &m).unwrap();
        assert!(TC64::ver_comm(
            &time_pp,
            &vk,
            &comm,
            &proof,
    ...
    fn snark_tc_512_test() {
        ...
        let (pk, vk) = Groth16::<Bls12_381>::circuit_specific_setup(Circuit512::default(), &mut rng).unwrap();


        let (comm, self_opening, proof) = TC512::commit(&mut rng, &time_pp, &ped_pp, &pk, &m).unwrap();
        assert!(TC512::ver_comm(
            &time_pp,
            &vk,
            &comm,
            &proof,
    ...
    fn snark_tc_2048_test() {
        ...
        let (pk, vk) = Groth16::<Bls12_381>::circuit_specific_setup(Circuit::default(), &mut rng).unwrap();
