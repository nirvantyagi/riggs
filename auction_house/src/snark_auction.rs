...
use rsa::bigint::constraints::BigIntCircuitParams;
use rsa::{
...
use timed_commitments::{
  basic_tc::TimeParams,
  lazy_tc::LazyTC,
  snark_tc::{Comm as TCComm, Opening as TCOpening, SnarkTC, SnarkTCParams, TCCircuit},
  ...
  Auction<G, PoEP, RsaP, H, H2P>
{
  ...
  ) -> Result<(TCComm<G, RsaP>, TCOpening<G, RsaP, H2P>, PS::Proof), Error> {
    TC::commit(rng, &pp.time_pp, &pp.ped_pp, &pk, &bid.to_le_bytes())
  }
  ...
  pub fn force_open_bid(
    ...
    let (bid_bytes, opening) = TC::force_open(
      &pp.time_pp,
      &pp.ped_pp,
      self
        .bid_comms_i
