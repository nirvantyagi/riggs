...
use rsa::{
    bigint::{
        constraints::{BigIntCircuitParams},
    },
      ...
      GV: CurveVar<G, F>,
{
    ...
    pub fn client_create_bid<R: CryptoRng + Rng>(
        ...
        bid: u32,
    ) -> Result<(SnarkTCComm<G, RsaP>, SnarkTCOpening<G, RsaP, H2P>, PS::Proof), Error> {
        SnarkTC::<F, PS, P, PoEP, RsaP, IntP, H2P, G, GV>::commit(rng, &pp.time_pp, &pp.ped_pp, &pp.snark_pk, &bid.to_le_bytes())
    }
