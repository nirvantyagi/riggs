...
use crate::{
    snark_auction::{SnarkAuction as Auction, AuctionParams, AuctionPhase},
    AuctionError, Error,
};
use range_proofs::bulletproofs::{Bulletproofs, Params as RangeProofParams, Proof as RangeProof};
use rsa::{
    bigint::{nat_to_f, BigInt, constraints::BigIntCircuitParams},
...
pub struct AuctionHouse<
    ...
    H: Digest,
> {
    active_auctions: HashMap<u32, (Auction<F, PS, P, PoEP, RsaP, IntP, H2P, G, GV>, HashMap<u32, u32>)>, // auction_id -> (auction, (user_id -> bid_id))
...
impl<F, PS, P, PoEP, RsaP, IntP, H2P, G, GV, H> AccountPrivateState<F, PS, P, PoEP, RsaP, IntP, H2P, G, GV, H>
    where
        F: PrimeField,
        PS: SNARK<F>,
        P: SnarkTCParams<F>,
        ...
        H: Digest,
{
    ...
    pub fn propose_bid<R: CryptoRng + Rng>(
        ...
        let (comm_bid, opening_bid, comm_proof) =
            Auction::<F, PS, P, PoEP, RsaP, IntP, H2P, G, GV>::client_create_bid(rng, &auction_pp.auction_pp, bid)?;
        // Prove bid > 0
...
impl<F, PS, P, PoEP, RsaP, IntP, H2P, G, GV, H> AuctionHouse<F, PS, P, PoEP, RsaP, IntP, H2P, G, GV, H>
    where
        F: PrimeField,
        PS: SNARK<F>,
        P: SnarkTCParams<F>,
...
mod tests {
    ...
    fn basic_snark_auction_house_test() {
        ...
        // Bid on auction 1
        let bid = 100;
        let (proposal, opening) = users.get_mut(0).unwrap()
        ...
        auction_house
            ...
            .unwrap();
        users.get_mut(0).unwrap().confirm_bid(
            &house_pp,
            &auction1_pp,
            auction1_id,
            bid,
        ...
        auction_house
            ...
            .unwrap();
        users.get_mut(0).unwrap().confirm_bid_self_open(&house_pp, &auction1_pp).unwrap();
