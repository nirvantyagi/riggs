...
use crate::{
  snark_auction::{Auction, AuctionParams, AuctionPhase},
  AuctionError, Error,
};
...
use timed_commitments::{
  snark_tc::{Comm as TCComm, Opening as TCOpening},
  PedersenParams,
};
  ...
  AccountPrivateState<G, PoEP, RsaP, H, H2P>
{
  ...
  pub fn propose_bid<R: CryptoRng + Rng>(
    ...
    let (comm_bid, opening_bid) =
      Auction::<G, PoEP, RsaP, H, H2P>::client_create_bid(rng, &auction_pp.auction_pp, bid, pk)?;
    // Prove bid > 0
  ...
  pub fn confirm_withdrawal(&mut self, _house_pp: &HouseParams<G>, amt: u32) -> Result<(), Error> {
    self.public_summary.balance -= amt;
    Ok(())
  }
}
