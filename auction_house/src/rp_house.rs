...
#[derive(Clone, PartialEq, Eq, Debug)]
pub struct HouseAuctionParams<G: ProjectiveCurve> {
    pub auction_pp: AuctionParams<G>,
...
pub struct AccountPrivateState<G: ProjectiveCurve, H: Digest> {
    ...
    _auction: PhantomData<Auction<G, H>>,
}

pub struct BidProposal<G: ProjectiveCurve> {
    pub comm_bid: PedComm<G>,
...
impl<G: ProjectiveCurve, H: Digest> AccountPrivateState<G, H> {
    ...
    pub fn propose_bid<R: CryptoRng + Rng>(
        ...
        let (comm_bid, opening_bid) =
            Auction::<G, H>::client_create_bid(rng, &auction_pp.auction_pp, bid)?;
        // Prove bid > 0
        ...
        let range_proof_balance = Bulletproofs::<G, H>::prove_range(
            ...
            &(opening_bid.neg() - &self.opening_active_bids),
            BID_BITS as u64,
        )?;
        Ok((
            BidProposal {
    ...
    pub fn confirm_bid(
        ...
        auction_id: u32,
        bid: u32,
        proposal: &BidProposal<G>,
        opening: &G::ScalarField,
    ) -> Result<(), Error> {
        // Update balance to remove reward
        self.public_summary.balance -= auction_pp.reward_self_open;
        // Update active bids
        self.sum_active_bids += bid;
        self.opening_active_bids += opening;
        self.public_summary.comm_active_bids += &proposal.comm_bid.g;
    ...
    pub fn confirm_auction_win(
        &mut self,
        _house_pp: &HouseParams<G>,
        _auction_pp: &HouseAuctionParams<G>,
        auction_id: u32,
        price: u32,
    ) -> Result<(), Error> {
        {
            ...
            self.sum_active_bids -= bid;
            self.opening_active_bids -= opening;
            self.public_summary.comm_active_bids -= bid_comm.g;
        }
...
impl<G: ProjectiveCurve, H: Digest> AuctionHouse<G, H> {
    ...
    pub fn account_bid(
        ...
        user_summary.comm_active_bids += &bid.comm_bid.g;
        Ok(())
    }
