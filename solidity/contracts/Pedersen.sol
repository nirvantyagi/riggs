...
library Pedersen  {
  ...
  // Versions that take Public Parameters as input.
  function commit(Params memory pp, uint b, uint r) internal view returns (BN254.G1Point memory) {
    return BN254.g1add(BN254.g1mul(pp.G, b), BN254.g1mul(pp.H, r));
  }

  function verify(Params memory pp, BN254.G1Point memory given, uint b, uint r) internal view returns (bool) {
    BN254.G1Point memory calc = commit(pp, b, r);
    return given.X==calc.X && given.Y==calc.Y; 
  }
