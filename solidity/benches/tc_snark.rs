...
use ark_groth16::{generator::generate_random_parameters, Groth16};
...
use solidity::{
    encode_groth16_input, encode_groth16_inputs, encode_groth16_inputs_struct,
    encode_groth16_proof, encode_tc_comm, encode_tc_opening, encode_tc_pp, get_bigint_library_src,
    get_bn254_library_src, get_filename_src, get_fkps_src, get_pedersen_library_src,
    get_rsa_library_src, read_groth16_src,
};
...
fn main() {
    ...
    println!(
        "Groth16 contract deployed at address: {:?}",
        groth16_lib_addr
    );
    ...
    let result = evm
        .call(
            groth16_contract
                .encode_call_contract_bytes("verify", &[encoded_inputs, encoded_proof])
                .unwrap(),
            &groth16_lib_addr,
            &deployer,
        )
