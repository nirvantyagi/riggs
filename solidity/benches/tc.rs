...
use solidity::{
    encode_tc_comm, encode_tc_opening, encode_tc_pp, get_bigint_library_src, get_bn254_library_src,
    get_filename_src, get_fkps_src, get_pedersen_library_src, get_rsa_library_src,
};
    ...
    for tamper_method in tamper_force_open.into_iter() {
        ...
        let (tc_comm, m, tc_force_opening) = match tamper_method.as_str() {
            "bad_x" => {
                ...
                tc_input_group_element_bad.tc_comm.x =
                    RsaHiddenOrderGroup::from_nat(BigInt::from(3));
