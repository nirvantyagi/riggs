...
use solidity::{
  encode_bulletproof, encode_new_auction, encode_ped_comm_struct,
...
use solidity_test_utils::{address::Address, contract::Contract, evm::Evm, to_be_bytes,
                          encode_group_element, encode_field_element,};
...
fn main() {
  ...
  println!("Compiling RP auction house contract...");
  ...
  // Complete auction
  {
    ...
    let result = evm
      .call(
        erc721_contract
          .encode_call_contract_bytes("balanceOf", &[bidders.get(1).unwrap().1.as_token()])
          .unwrap(),
        &erc721_contract_addr,
        &deployer,
      )
